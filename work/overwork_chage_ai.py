    ...
    try:
        ...
        for i, record in enumerate(llm_records):
            try:
                ...
                # 更频繁地输出进度，每10条记录更新一次
                if (i+1) % 10 == 0 or i+1 == len(llm_records):
        ...
        # 将失败记录和未找到员工的记录保存到Excel文件
        if failed_records or not_found_records:
            try:
                # 创建输出目录（如果不存在）
                ...
                excel_file = os.path.join(output_dir, f'失败的加班记录_{current_time}.xlsx')
                ...
                with pd.ExcelWriter(excel_file) as writer:
                    ...
                    if not_found_records:
                        not_found_df = pd.DataFrame([
                            {
                                '姓名': record['record'][0] if len(record['record']) > 0 else '未知',
                                '日期': record['record'][1] if len(record['record']) > 1 else '未知',
                                '时间': record['record'][2] if len(record['record']) > 2 else '未知',
                                '时长': record['record'][3] if len(record['record']) > 3 else '未知',
                                '加班说明': record['record'][4] if len(record['record']) > 4 else '未知',
                                '来源': record['record'][5] if len(record['record']) > 5 else '未知',
                                '错误类型': '未找到员工'
                            } for record in not_found_records
                        ])
                        not_found_df.to_excel(writer, sheet_name='未找到员工', index=False)
                    ...
                    if failed_records:
                        ...
                        failed_df.to_excel(writer, sheet_name='处理失败', index=False)
                
                logger.log_info(f"失败记录已保存到Excel文件: {excel_file}")
                summary_info.append(f"\n失败记录已保存到Excel文件: {excel_file}")
            except Exception as e:
                logger.log_error(f"保存失败记录到Excel文件时出错: {e}")
                summary_info.append(f"\n保存失败记录到Excel文件时出错: {e}")
